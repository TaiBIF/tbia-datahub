...
for i in df.index:
    ...
    response = requests.post('http://solr:8983/solr/tbia_records/select?stats=true&stats.field=standardDate&facet.field=resourceContacts&facet.field=recordType&facet.limit=-1&facet.mincount=1&facet=true', data=json.dumps(query), headers={'content-type': "application/json" })
    resp = response.json()
    ...
    record_list = [data[x] for x in range(0, len(data),2)]
    record_type = ','.join(record_list)
...
for i in df.index:
    ...
    query = '''UPDATE dataset SET "datasetTaxonGroup" = %s, "datasetTaxonStat" = %s, "occurrenceCount" = %s, 
                "datasetDateStart" = %s, "datasetDateEnd" = %s, "resourceContacts" = %s, "record_type" = %s WHERE "tbiaDatasetID" = %s;
            '''
    with conn.cursor() as cursor:
        execute_line = cursor.execute(query, (row.datasetTaxonGroup, row.datasetTaxonStat, row.occurrenceCount, row.datasetDateStart, row.datasetDateEnd, row.resourceContacts, row.record_type, row.tbiaDatasetID))
        conn.commit()
...
keys =  ['id','name','record_type','rights_holder','deprecated','datasetTaxonGroup','resourceContacts','sourceDatasetID','gbifDatasetID','tbiaDatasetID','occurrenceCount','datasetURL','datasetLicense','created','modified','datasetPublisher','update_version','datasetDateStart','datasetDateEnd','datasetTaxonStat','downloadCount','group' ]
...
# a = df[df.deprecated==False][['name']].groupby('name',as_index=False).count()

now = datetime.now().strftime('%Y%m%d')

updating_csv = '/bucket/tbia_updated_dataset_{}.csv'.format(now)
