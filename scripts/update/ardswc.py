...
while has_more_data:
    ...
    if len(data):
        ...
        # 如果學名相關的欄位都是空值才排除
        df = filter_by_taxon_fields(df, required_cols=['isPreferredName','scientificName','class','order','family'])
        ...
        if len(df):
            ...
            df = df.drop(columns=[ 'kingdom_c', 'phylum', 'phylum_c', 'class_c',
                                   'order_c', 'family_c', 'genus', 'genus_c'])
            df = process_taxon_match(df, sci_cols)
            df = apply_common_fields(df, group, rights_holder, now)
            df = apply_record_type(df, mode='occ') # basisOfRecord是錯誤資訊
            df, media_rule_list = apply_media_rule(df, [])
            df[geo_keys] = process_geo_batch(df, is_full_hidden='auto', infer_generalizations=True)
            df['datasetName'] = df['datasetName'].fillna('集水區友善環境生態資料庫').replace('', '集水區友善環境生態資料庫')
            df = df.replace(to_quote_dict)
