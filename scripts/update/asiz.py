...
while has_more_data:
    ...
    if len(data):
        ...
        # 有些資料沒有 補上
        cols_to_ensure = ['familyName', 'isPreferredName']
        df = df.reindex(columns=df.columns.union(cols_to_ensure), fill_value='')
        # 如果學名相關的欄位都是空值才排除
        df = filter_by_taxon_fields(df, required_cols=['isPreferredName','scientificName','familyName'])
        ...
        if len(df):
            ...
            # familyName 是英文+中文
            df['sourceFamily'] = df['sourceFamily'].str.split().str[0]
